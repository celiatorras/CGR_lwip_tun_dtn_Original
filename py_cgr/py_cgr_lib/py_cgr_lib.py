...
class Contact:
    def __init__(self, time_now, frm, to, start, end, rate, confidence=1., owlt=0):
        # fixed parameters
        self.frm = frm
        self.to = to
        self.start = start + time_now
        self.end = end + time_now
        self.rate = rate
        self.owlt = owlt
...
def cgr_yen(time_now, source, destination, curr_time, contact_plan, num_routes):
    ...
    # create Root Contact
    root_contact = Contact(time_now, source, source, 0, sys.maxsize, 100, 1.0, 0)  # root contact
